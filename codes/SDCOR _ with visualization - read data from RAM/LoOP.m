...
% Calculating AUC
[~,~,~,AUC_LoOP] = perfcurve(H.labFin,LoOPvals,1);
...
% Nested function for calculating the neighborhood graph block-by-block
    function matlMat_Maker()
        ...
        for c1 = 1:cntLim
            %     c1
            if c1 ~= cntLim
                Y1 = H.DS((c1-1)*BLK_SZ_LIM+1:c1*BLK_SZ_LIM,:);
            else
                Y1 = H.DS((c1-1)*BLK_SZ_LIM+1:end,:);
            end
            chnkSz = size(Y1,1);
            matlMatY = [];
            matlMatYind = [];
        ...
        function plotProgrsBars()
            axes(H.axes2);
            bar([cntLim.*ones(1,c1-1) c2 zeros(1,cntLim-c1+1)],'g');
            pause(.01);
        end
